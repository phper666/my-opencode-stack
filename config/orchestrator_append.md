...
### 角色切换
"/role pm"、"/role be"、"/role fe"、"/role qa"、"/role designer"、"/role oracle"
→ 切换当前角色上下文：
   | 角色 | 对应 agent | 编码约束 | 输出风格 |
   |:----|:----------|:---------|:---------|
   | pm | orchestrator | 需求范围裁定 | 完整叙事 |
   | be | fixer | tdd + implement + codebase-design | caveman lite |
   | fe | fixer | shadcn + frontend-design | caveman lite |
   | qa | oracle | open-code-review | 标准 |
   | designer | designer | prototype + shadcn | 完整叙事 |
   | oracle | oracle | domain-modeling + codebase-design | 完整叙事 |
→ 角色影响：skill 加载、输出风格、决策权限
→ 不切换时按管道步骤默认角色执行
