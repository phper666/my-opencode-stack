...
实现完成（步骤 5）
  ↓
┌──────────────────────────┐
│ 步骤 6: 自修复 lint       │  3 轮迭代：--fix → agent修 → 验证
│  tsc / eslint / clippy   │  编译 error → 退回步骤 5
│  cargo check / ruff 等   │  warning 可 suppress 并记录
└──────────┬───────────────┘
           ↓
┌──────────────────────────┐
│ 步骤 7: Code Review       │  ocr review --audience agent → 修 high/medium
│  (open-code-review)       │  重审 ≤3 轮 → @oracle 裁定
│  + ponytail-review        │  再加过度设计审查（可简化项列表）
└──────────┬───────────────┘
           ↓
┌──────────────────────────┐
│ 步骤 8: 安全扫描           │  semgrep --config=auto
│  (semgrep)               │  SQL注入 / XSS / 硬编码密钥
└──────────┬───────────────┘  重扫 ≤3 轮 → @oracle 裁定
           ↓
┌──────────────────────────┐
│ 步骤 9: PRD 回验           │  @oracle 逐项核对 PRD 需求
│  (oracle)                │  + TDD 合规检查 + 设计对照
└──────────┬───────────────┘  重验 ≤3 轮 → @oracle 裁定
           ↓
        ✅ 可合并
```
